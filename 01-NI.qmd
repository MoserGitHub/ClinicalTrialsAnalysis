...
In the former subsection we assumed that the prior was directly specified on the treatment effect. In this subsection we assume that that design prior is specified on the event probabilities $p_i$, $i=\{0,1\}$. Suppose that the design priors on the event probabilities are beta distributed $\pi_i \sim Beta(a_i,b_i)$. It is well known (see for example @Spiegelhalter2003 or @Grieve2022) that the prior predictive distribution of $n_i$ new Bernoulli observations is beta-binomial distributed with
$$
E(\tilde Y_i)=n_i\frac{a_i}{a_i+b_i}, \quad \sigma_{i,\pi_i}^2=\frac{\left(a_i+\sum_{k\leq n_i}Y_{i,k}\right)\left(b_i+n_i-\sum_{k\leq n_i}Y_{i,k}\right)}{(a_i+b_i+n_i)^2(a_i+b_i+n_i+1)}.
$$
...
**Enthusiastic prior**: The beta prior for the active treatment arm and control arm is centered at 0.01 (the expected event proportion) with $P(\pi_1\leq 0.05)=0.01$ and $P(\pi_1\leq 0.05)=0.05$.
...
prior_plot_long <- pivot_longer(prior_plot, cols=c(prior_0, prior_1, posterior_0, posterior_1))
prior_plot_long$type <- ifelse(str_detect(prior_plot_long$name, "posterior")==T, "Posterior", "Prior")
prior_plot_long$group <- ifelse(str_detect(prior_plot_long$name, "_0")==T, "Treatment arm 0", "Treatment arm 1")

ggplot(prior_plot_long, aes(x=x, y=value, linetype=type, colour=group))+geom_line()+theme_bw()+theme(panel.grid.minor = element_blank(), legend.position = "bottom", legend.direction = "vertical")+scale_x_continuous(limits=c(0,0.1))+ylab("Density")+labs(caption=str_glue("Assumed event probabilities: Arm 0 (p_0=", p_0, "), ", "Arm 1 (p_1=", p_1, ")\nPosterior standard deviation: Arm 0=", round(sqrt(var_posterior_0), 3), ", ", "Arm 1=", round(sqrt(var_posterior_1), 3)))+scale_color_brewer("Treatment arm", palette="Set2")+scale_linetype("Type")+ggtitle("Enthusiastic prior")
```

**Skeptical prior**: The beta prior for the active treatment arm is centered at 0.05 (the upper margin for safety) with $P(\pi_1\leq 0.01)=0.025$. The mean of the beta prior for the control arm is chosen such that the mean difference between both priors lies on the non-inferiority margin and $P(\pi_0> 0.05)=0.025$.

- Parameters of beta prior for active treatment arm: $a_1=2.84$, $b_1=53.96$.
- Parameters of beta prior for control treatment arm: $a_i=1.24$, $b_i=81.43$.
...
ggplot(prior_plot_long, aes(x=x, y=value, linetype=type, colour=group))+geom_line()+theme_bw()+theme(panel.grid.minor = element_blank(), legend.position = "bottom", legend.direction = "vertical")+scale_x_continuous(limits=c(0,0.1))+ylab("Density")+labs(caption=str_glue("Expected event probabilities: Arm 0 (p_0=", p_0, "), ", "Arm 1 (p_1=", p_1, ")\nPosterior standard deviation: Arm 0=", round(sqrt(var_posterior_0), 3), ", ", "Arm 1=", round(sqrt(var_posterior_1), 3)))+scale_color_brewer("Treatment arm", palette="Set2")+scale_linetype("Type")+ggtitle("Skeptical prior")
```
...
ggplot(prior_plot_long, aes(x=x, y=value, linetype=type, colour=group))+geom_line()+theme_bw()+theme(panel.grid.minor = element_blank(), legend.position = "bottom", legend.direction = "vertical")+scale_x_continuous(limits=c(0,0.1))+ylab("Density")+labs(caption=str_glue("Assumed event probabilities: Arm 0 (p_0=", p_0, "), ", "Arm 1 (p_1=", p_1, ")\nPosterior standard deviation: Arm 0=", round(sqrt(var_posterior_0), 3), ", ", "Arm 1=", round(sqrt(var_posterior_1), 3)))+scale_color_brewer("Treatment arm", palette="Set2")+scale_linetype("Type")+ggtitle("Noninformative prior")
```
...
set.seed(1)
data_points$pdf_r_1 <- dbbinom(data_points$r_1, size=ceiling(n_1), a_1, b_1)
data_points$pdf_r_0 <- dbbinom(data_points$r_0, size=ceiling(n_0), a_0, b_0)
data_points$joint_pdf <- data_points$pdf_r_1*data_points$pdf_r_0

data_points$sig <- NA

for (i in 1:nrow(data_points)) {
data_points$sig[i] <- ifelse(wald2ci(data_points$r_1[i], n_1, data_points$r_0[i], n_0, conf.level=0.95, adjust="AC")$conf.int[2]<=delta_star, 1, 0)
}

res_ap <- data.frame(type="Enthusiastic", ap=sum(data_points$joint_pdf[data_points$sig==1]))
...
res_ap <- rbind(res_ap, data.frame(type="Skeptical", ap=sum(data_points$joint_pdf[data_points$sig==1])))
...
res_ap <- rbind(res_ap, data.frame(type="Noninformative prior", ap=sum(data_points$joint_pdf[data_points$sig==1])))
res_ap$ap <- round(res_ap$ap, 2)

res_ap
```

##### Simulation approach 2: Direct sampling from predictive distribution
...
set.seed(1)
r_1 <- rbbinom(10000, size=ceiling(n_1), a_1, b_1)
r_0 <- rbbinom(10000, size=ceiling(n_0), a_0, b_0)

res <- c()

for (i in 1:length(r_1)) {
  res <- c(res, ifelse(wald2ci(r_1[i], n_1, r_0[i], n_0, conf.level=0.95, adjust="AC")$conf.int[2]<=delta_star, 1, 0))
}

res_ap <- data.frame(type="Enthusiastic", ap=sum(res)/10000)
...
res_ap <- rbind(res_ap, data.frame(type="Skeptical", ap=sum(res)/10000))
...
res_ap <- rbind(res_ap, data.frame(type="Noninformative", ap=sum(res)/10000))
...
Denote the random variable $D^\pi:=p_1|\pi_1-p_0|\pi_0$ and $\delta^\pi:=\mu_{1,\pi_1}-\mu_{0,\pi_0}$, then, since $D_\pi \sim N(\delta^\pi, \sigma_{1,\pi_1}^2+\sigma_{0,\pi_0}^2)$, one gets a significant result if
$$
D^\pi+z_{1-\alpha}\sqrt{\sigma_{1,\pi_1}^2+\sigma_{0,\pi_0}^2}\leq \delta^*.
$$
Denote $D^\pi_{suc}:=-z_{1-\alpha}\sqrt{\sigma_{1,\pi_1}^2+\sigma_{0,\pi_0}^2}+\delta^*$

Then
$$
P(D^\pi\leq D^\pi_{suc}|\delta^\pi)=\Phi\left( z_{1-\alpha} -\frac{\delta^\pi-\delta^*}{\sqrt{\sigma_{1,\pi_1}^2+\sigma_{0,\pi_0}^2}}\right)
$$
