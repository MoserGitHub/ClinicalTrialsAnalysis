...
$$
and thus, by using Fubini's theorem, the incomplete beta function $B(x;a,b)$ and the guidance of Patricia Altham @Altham1969,
$$
\begin{aligned}
...
&\overset{u=\pi_0-\delta^*}{=}\int_{0}^{1-\delta^*}\frac{u^{a_0+r_0-1}(1-u)^{b_0+n_0-r_0-1}}{B(a_0+r_0, b_0+n_0-r_0)}\left(\int_0^{u} \frac{\pi_1^{a_1+r_1-1}(1-\pi_1)^{b_1+n_1-r_1-1}}{B(a_1+r_1, b_1+n_1-r_1)}d\pi_1\right)du \\
&=\int_{0}^{1-\delta^*}\frac{u^{a_0+r_0-1}(1-u)^{b_0+n_0-r_0-1}}{B(a_0+r_0, b_0+n_0-r_0)}\left(\sum_{s=a_1+r_1}^{a_1+b_1+n_1-1}\begin{pmatrix} a_1+b_1+n_1-1 \\ s \end{pmatrix}u^s(1-u)^{a_1+b_1+n_1-2-s}\right)du \\
&=\sum_{s=a_1+r_1}^{a_1+b_1+n_1-1}\begin{pmatrix} a_1+b_1+n_1-1 \\ s \end{pmatrix}\frac{1}{B(a_0+r_0, b_0+n_0-r_0)}\left(\int_{0}^{1-\delta^*}u^{a_0+r_0+s-1}(1-u)^{a_1+b_1+n_1-2+b_0+n_0-r_0-s-1}du\right) \\
&=\sum_{s=a_1+r_1}^{a_1+b_1+n_1-1}\begin{pmatrix} a_1+b_1+n_1-1 \\ s \end{pmatrix}\frac{B(1-\delta^*; a_0+r_0+s, a_1+b_1+n_1-2+b_0+n_0-r_0-s)}{B(a_0+r_0, b_0+n_0-r_0)}, \\
\end{aligned}
$$
is the **Bayesian probability to reject** given assumed $r_0, r_1, n_0, n_1$. Grieve highlights that this is can be written as a hypergeometric distribution (@Grieve2022, @Grieve2016)
$$
\begin{aligned}
\sum_{s=\max(r_0+a_0-r_1-a_1,0)}^{r_0+a_0-1} \frac{\begin{pmatrix} r_0+r_1+a_0+a_1-1 \\ s \end{pmatrix}\begin{pmatrix} n_0+n_1-r_0-r_1+b_0+b_1-1 \\ n_0+a_0+b_0-1-s \end{pmatrix}}{\begin{pmatrix} n_0+n_1+a_0+a_1+b_0+b_1-2 \\ n_1+a_1+b_1-1 \end{pmatrix}}
\end{aligned}
$$
...
bap <- integrate(inner_int, 0, 1-delta_star)$value/(beta(a_1 +
