...
#### Hybrid approach: Prior on the treatment effect
...
be the **'average power'** @Grieve2022 (also called 'assurance' @OHagan2005, 'probability of success' @Kunzmann2022, @Spiegelhalter2003 or Bayesian predictive power @Rufibach2016. @Kunzmann2022 provide in their supplemental section a literature review of the used terminology in publications from clinical trials).

Remember that in an hybrid approach we are interested in trial conclusions from a frequentist point of view, thus we are interested in
$$
...
$$ since $\tilde D \sim N\left(\delta, \sigma_{treat}^2\right)$, see for example @Gelman2013.
...
#### Bayesian approach: Prior on the treatment effect
...
$$
where $\epsilon$ is small, say $\epsilon=0.05$. Similar to the frequentist approach one is interested whether the upper $(1-\epsilon)$ credible interval of $D$ (taking into account the prior information) is smaller than the non-inferiority margin $\delta^*$, that is
$$
...
pred_0 <- dbbinom(x = 1:999, size = 1000, a_0, b_0) *
    1000
pred_1 <- dbbinom(x = 1:999, size = 1000, a_1, b_1) *
    1000

prior_plot <- data.frame(x, prior_0, prior_1, pred_0,
    pred_1)
prior_plot_long <- pivot_longer(prior_plot, cols = c(prior_0,
    prior_1, pred_0, pred_1))
...
prior_plot_long$type <- ifelse(str_detect(prior_plot_long$name,
    "pred_") == T, "Predicted", "Prior")

fig1 <- ggplot(prior_plot_long, aes(x = x, y = value,
    colour = group, linetype = type)) + geom_line(linewidth = 1.5) +
    theme_bw() + theme(panel.grid.minor = element_blank(),
    legend.position = "bottom", legend.direction = "horizontal") +
    scale_x_continuous(limits = c(0, 0.1)) + ylab("Density") +
    labs(caption = str_glue("Assumed event probabilities: Control (p_0=",
...
fig2 <- ggplot(diff_prior, aes(x), group = x) + geom_density(linewidth = 1.5,
    ...
    ylab("Density") + geom_vline(xintercept = delta_star,
    linetype = "dashed") + geom_vline(xintercept = 0,
    linetype = "dotted") + scale_x_continuous(breaks = c(-1, -0.5,
    -0.25, 0, 0.035, 0.25, 0.5, 1), labels = c(-1, -0.5, -0.25,
    0, 0.035, 0.25, 0.5, 1), limits = c(-1, 1))
...
##### Simulation approach 1: Joint probability
...
##### Simulation approach 2: Direct sampling from prior predictive distribution

The second simulation approach samples $q$ event outcomes from the predictive distribution and calculates whether the $(1-\alpha)$-upper confidence limit of the risk difference is smaller than the non-inferiority margin. The sum of positive outcomes divided by the number of samples $q$ gives the average power. Also here we use Agresti-Caffo confidence intervals for the risk difference @Agresti2000.
...
prior_plot_long$group <- ifelse(str_detect(prior_plot_long$name,
    "_0") == T, "Treatment arm 0", "Treatment arm 1")
...
ggplot(prior_plot_long, aes(x = x, y = value, linetype = type,
    ...
    scale_x_continuous(limits = c(0, 0.1)) + ylab("Density") +
    labs(caption = str_glue("Assumed event probabilities: Arm 0 (p_0=",
        p_0, "), ", "Arm 1 (p_1=", p_1, ")\nPosterior standard deviation: Arm 0=",
        round(sqrt(var_posterior_0), 3), ", ", "Arm 1=",
...
$$
and thus, by using Fubini's theorem and the guidance of Patricia Altham @Altham1969,
$$
\begin{aligned}
...
\end{aligned}
$$
is the **Bayesian probability to reject** given assumed $r_0, r_1, n_0, n_1$. This integral can be solved using the *integrate* function in R, or using the continuation of the formula
$$
\begin{aligned}
\cdots&=\int_{-\delta^*}^{1-\delta^*}\frac{u^{a_0+r_0-1}(1-u)^{b_0+n_0-r_0-1}}{B(a_0+r_0, b_0+n_0-r_0)}\left(\sum_{s=a_1+r_1}^{a_1+b_1+n_1-1}\begin{pmatrix} a_1+b_1+n_1-1 \\ s \end{pmatrix}u^s(1-u)^{a_1+b_1+n_1-1-s}\right)du \\
...
where the last term in the above formula is the upper tail of a beta-binomial distribution. Altham @Altham1969 showed that this can be written as a hypergeometric distribution (see also @Grieve2022, @Grieve2016).

------------------------------------------------------------------------

**Working example (continued)**

We calculate the Bayesian probability to reject for the SAFE-SSPE study. We show the integral solution and the calculation from the upper tail of a beta-binomial distribution.

Note that for the integral calculation the lower integrand $-\delta^*$ in

$$
\int_{-\delta^*}^{1-\delta^*}\frac{u^{a_0+r_0-1}(1-u)^{b_0+n_0-r_0-1}}{B(a_0+r_0, b_0+n_0-r_0)}\left(\int_0^{u} \frac{\pi_1^{a_1+r_1-1}(1-\pi_1)^{b_1+n_1-r_1-1}}{B(a_1+r_1, b_1+n_1-r_1)}d\pi_1\right)du
$$
needs to be zero, otherwise the function is not defined.
...
inner_func <- function(x) {
    x^(a_1 + ceiling(n_1) * p_1 - 1) * (1 - x)^(b_1 +
        ceiling(n_1) - ceiling(n_1) * p_1 - 1)
}

inner_int <- Vectorize(function(u) {
    u^(a_0 + ceiling(n_0) * p_0 - 1) * (1 - u)^(b_0 +
        ceiling(n_0) - ceiling(n_0) * p_0 - 1) * integrate(inner_func,
        0, u)$value
})
...
bap_integral <- integrate(inner_int, 0, 1-delta_star)$value/(beta(a_1 +
    n_1 * p_1, b_1 + n_1 - n_1 * p_1) * beta(a_0 + n_0 * p_0, b_0 + n_0 -
    n_0 * p_0))

s <- ceiling(seq(a_1 + n_1 * p_1, b_1 + n_1 - n_1 *
    p_1 - 1))
bap_tail_betabinomial <- sum(dbbinom(s, size = ceiling(a_1 +
    b_1 + n_1 - 1), alpha = a_0 + n_0 * p_0, beta = b_0 +
    n_0 - n_0 * p_0))

res_bap <- data.frame(type="Enthusiastic", bap_integral, bap_tail_betabinomial)
...
res_bap <- rbind(res_bap, data.frame(type="Skeptical", bap_integral, 
...
res_bap <- rbind(res_bap, data.frame(type="Informative", bap_integral, 
                                     bap_tail_betabinomial))
...
res_bap <- rbind(res_bap, data.frame(type="Noninformative", bap_integral, 
                                     bap_tail_betabinomial))

res_bap$bap_integral <- round(res_bap$bap_integral, 2)
res_bap$bap_tail_betabinomial <- round(res_bap$bap_tail_betabinomial, 2)

res_bap
```

Small differences between both approaches occur because the 'beta-binomial distribution' approach is a discretization of the integral approach and requires integer steps for the calculation.
