...
\begin{aligned}
P&\left(p_1-p_0 \leq \delta^*\right|r_0, r_1, n_0, n_1)=\int_0^{1}\int_0^{p_0+\delta^*} f_2(p_1, p_0|r_0, r_1, n_0, n_1)dp_1dp_0 \\
&=\int_0^{1}\frac{p_0^{a_0+r_0-1}(1-p_0)^{b_0+n_0-r_0-1}}{B(a_0+r_0, b_0+n_0-r_0)}\left(\int_0^{p_0+\delta^*} \frac{p_1^{a_1+r_1-1}(1-p_1)^{b_1+n_1-r_1-1}}{B(a_1+r_1, b_1+n_1-r_1)}dp_1\right)dp_0 \\
&\overset{u=p_0-\delta^*}{=}\int_{-\delta^*}^{1-\delta^*}\frac{u^{a_0+r_0-1}(1-u)^{b_0+n_0-r_0-1}}{B(a_0+r_0, b_0+n_0-r_0)}\left(\int_0^{u} \frac{p_1^{a_1+r_1-1}(1-p_1)^{b_1+n_1-r_1-1}}{B(a_1+r_1, b_1+n_1-r_1)}dp_1\right)du \\
...
data_points$bap <- NA
for (i in 1:nrow(data_points)) {
s <- ceiling(seq(a_1 + data_points$r_1[i], b_1 + n_1 - data_points$r_1[i] - 1))
data_points$bap[i] <- sum(dbbinom(s, size = ceiling(a_1 +
    b_1 + n_1 - 1), alpha = a_0 + data_points$r_0[i], beta = b_0 +
    n_0 - data_points$r_0[i]))
}



for (i in 1:nrow(data_points)) {
    data_points$sig[i] <- ifelse(data_points$bap[i]<=1-alpha,
        1, 0)
}

res_bap <- data.frame(type = "Enthusiastic", 
                            ap = sum(data_points$joint_pdf[data_points$sig == 1]))
...
res_bap <- data.frame(type = "Non-informative", 
                            ap = sum(data_points$joint_pdf[data_points$sig == 1]))
