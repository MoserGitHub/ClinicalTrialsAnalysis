...
\end{aligned}
$$
is the **Bayesian probability to reject** given assumed $r_0, r_1, n_0, n_1$. Note that the last term in the above formula is the upper tail of a beta-binomial distribution Grieve highlights that this is can be written as a hypergeometric distribution (@Grieve2022, @Grieve2016).
...
# s <- seq(max(ceiling(n_0 * p_0+a_0-n_1*p_1-a_1),0), ceiling(n_1*p_1+a_0-(1-delta_star)))
# sum(dhyper(s, m=ceiling(n_0*p_0+n_1*p_1+a_1+a_0-1), n=ceiling(n_1+n_0-n_1*p_1-n_0 * p_0+b_1+b_0-1), k=ceiling(n_0+a_0+b_0-1)))
