...
The AP decomposition can be visualised using the posterior predictive distribution. Let $f(\tilde\delta)$ be the pdf of the posterior predictive distribution then $$
...
For the SAFE-SSPE study the bivariate scatterplot of $f_2(\tilde\delta, \delta)$ under an enthusiastic prior can be visualised as:
...
#### Proper Bayesian approach: Prior on the treatment effect
...
In the former subsections we assumed that the prior was directly specified on the treatment effect. In this subsection we assume that that design prior is specified on the event probabilities $p_i$, $i=\{0,1\}$. Suppose that the design priors on the event probabilities are beta distributed $\pi_i \sim Beta(a_i,b_i)$, $i=\{0,1\}$. It is well known (see for example @Spiegelhalter2003 or @Grieve2022) that the prior predictive distribution of the number of events $R_i$, $i=\{0,1\}$, from $n_i$ Bernoulli observations is beta-binomial distributed with $R_i \sim betabinom(r_i|n_i, a_i, b_i)$ and 
$$
...
$$
\pi_i|r_i \sim Beta\left(a_i+r_i,b_i+n_i-r_i\right)
$$ with
$$
...
Note that the posterior predictive distribution of a future binomial random sample $m_i$ is beta-binomial distributed $\tilde R_i \sim betabinom(\tilde r_i|m_i, n_i, a_i, b_i)$ with
$$
\mu_{i, \tilde R}:=E(\tilde R_i)=m_i\frac{a_i+r_i}{a_i+b_i+n_i}, \\ \sigma_{i, \tilde R}^2:=Var(\tilde R_i)=\frac{m_i(a_i+r_i)(b_i+n_i-r_i)(m_i+a_i+b_i+n_i)}{(a_i+b_i+n_i)^2(a_i+b_i+n_i+1)}.
$$
Suppose that $\rho_i$ is a realisation from $\tilde R_i$.
The difference between predicted event proportions $\tilde R_i/m_i$ is Gaussian distributed
$$
\tilde D=\int_{\tilde R_1}\frac{\rho_1}{m_1}d\rho_1-\int_{\tilde R_0}\frac{\rho_0}{m_1}d\rho_0 \sim N\left(\frac{\mu_{1, \tilde R}}{m_1}-\frac{\mu_{0, \tilde R}}{m_0}, \frac{\sigma_{1, \tilde R}^2}{m_1^2}+\frac{\sigma_{0, \tilde R}^2}{m_0^2}\right).
$$
Then one obtains a 'successful' trial result if
$$
\tilde D+z_{1-\epsilon}\sqrt{\frac{\sigma_{1, \tilde R}^2}{m_1^2}+\frac{\sigma_{0, \tilde R}^2}{m_0^2}}\leq \delta^*.
$$
The calculation of the Bayesian average power is again done by simulation.
...
set.seed(2)
r_1 <- rbbinom(q, size = m_1, a_1 + ceiling(n_1) * p_1,
    b_1 + ceiling(n_1) - ceiling(n_1) * p_1)
mean_r_1 <- m_1 * (a_1 + ceiling(n_1) * p_1)/(a_1 + ceiling(n_1) *
    p_1 + b_1 + ceiling(n_1) - ceiling(n_1) * p_1)
var_posterior_1 <- (m_1 * (a_1 + ceiling(n_1) * p_1) *
    (b_1 + ceiling(n_1) - ceiling(n_1) * p_1) * (m_1 +
    a_1 + ceiling(n_1) * p_1 + b_1 + ceiling(n_1) -
    ceiling(n_1) * p_1))/((a_1 + ceiling(n_1) * p_1 +
    b_1 + ceiling(n_1) - ceiling(n_1) * p_1)^2 * (a_1 +
    ceiling(n_1) * p_1 + b_1 + ceiling(n_1) - ceiling(n_1) *
    p_1 + 1))

r_0 <- rbbinom(q, size = m_0, a_0 + ceiling(n_0) * p_0,
    b_0 + ceiling(n_0) - ceiling(n_0) * p_0)
mean_r_0 <- m_0 * (a_0 + ceiling(n_0) * p_0)/(a_0 + ceiling(n_0) *
    p_0 + b_0 + ceiling(n_0) - ceiling(n_0) * p_0)
var_posterior_0 <- (m_0 * (a_0 + ceiling(n_0) * p_0) *
    (b_0 + ceiling(n_0) - ceiling(n_0) * p_0) * (m_0 +
    a_0 + ceiling(n_0) * p_0 + b_0 + ceiling(n_0) -
    ceiling(n_0) * p_0))/((a_0 + ceiling(n_0) * p_0 +
    b_0 + ceiling(n_0) - ceiling(n_0) * p_0)^2 * (a_0 +
    ceiling(n_0) * p_0 + b_0 + ceiling(n_0) - ceiling(n_0) *
    p_0 + 1))

data_bap <- data.frame(pred_1 = r_1/m_1, pred_0 = r_0/m_0,
    diff = r_1/m_1 - r_0/m_0, type = "Enthusiastic")

bap <- sum(ifelse(data_bap$diff + qnorm(1 - alpha) *
    sqrt(var_posterior_0/m_0^2 + var_posterior_1/m_1^2) <= delta_star,
    1, 0))/q

data_bap_graph <- data_bap

res_bap <- data.frame(type = "Enthusiastic", bap = bap)
...
data_bap <- data.frame(pred_1 = r_1/m_1, pred_0 = r_0/m_0,
    diff = r_1/m_1 - r_0/m_0, type = "Skeptical")

data_bap_graph <- rbind(data_bap_graph, data_bap)
...
data_bap <- data.frame(pred_1 = r_1/m_1, pred_0 = r_0/m_0,
    diff = r_1/m_1 - r_0/m_0, type = "Informative")
...
data_bap <- data.frame(pred_1 = r_1/m_1, pred_0 = r_0/m_0,
    diff = r_1/m_1 - r_0/m_0, type = "Noninformative")
...
ggplot(data_bap_graph, aes(x = pred_1 - pred_0)) + geom_density(linewidth = 1.5,
    bw = 0.01) + facet_wrap(~type) + geom_vline(xintercept = delta_star,
    linetype = "dashed") + theme_bw() + theme(panel.grid.minor = element_blank()) +
    geom_vline(xintercept = 0, linetype = "dotted") +
    xlab("Difference in posterior predicted event proportions") +
    ylab("Density") + scale_x_continuous(breaks = c(-0.1,
    0, 0.035, 0.1), labels = c(-0.1, 0, 0.035, 0.1)) +
    ggtitle("Difference in posterior predicted event proportions")
```