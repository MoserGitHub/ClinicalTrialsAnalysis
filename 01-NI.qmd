...
\begin{aligned}
P_\delta(D\leq D_{suc}^{\delta^*})&=P_\delta\left(\frac{D-\delta}{\sigma_{treat}}\leq\frac{-z_{1-\alpha}\sigma_{treat}+\delta^*-\delta}{\sigma_{treat}}\right) \\
&=\Phi\left(-z_{1-\alpha}-\frac{(\delta-\delta^*)}{\sigma_{treat}}\right) \\
&=\Phi\left(-z_{1-\alpha}-\sqrt{\frac{n_1n_0}{n_0\sigma_1^2+n_1\sigma_0^2}}(\delta-\delta^*)\right),
...
\end{aligned}
$$ since the predictive distribution of $\tilde D$ is Gaussian distributed $\tilde D \sim N\left(d, \sigma_{treat}^2+\sigma_{prior}^2\right)$ under a design prior $\Delta \sim N\left(d, \sigma_{prior}^2\right)$ (@Gelman2013, @Grieve2022). Note that as $m \rightarrow \infty$, then $AP \rightarrow \Phi\left(-z_{1-\alpha}-\frac{(d-\delta^*)}{\sigma_{treat}}\right)$, that is, the frequentist power at $d$.
...
$$ and thus, by using Fubini's theorem, $$
\begin{aligned}
...
&=\int_{\delta^*}^{1}\frac{p_1^{a_1+r_1-1}(1-p_1)^{b_1+n_1-r_1-1}}{B(a_1+r_1, b_1+n_1-r_1)}\left(\int_0^{p_1-\delta^*} \frac{p_0^{a_0+r_0-1}(1-p_0)^{b_0+n_0-r_0-1}}{B(a_0+r_0, b_0+n_0-r_0)}dp_0\right)dp_1 \\
&=\int_{\delta^*}^{1}\frac{p_1^{a_1+r_1-1}(1-p_1)^{b_1+n_1-r_1-1}}{B(a_1+r_1, b_1+n_1-r_1)}F_{B(a_0+r_0,b_0+n_0-r_0)}(p_1-\delta^*)dp_1,
\end{aligned}
$$ is the **Bayesian probability to reject** given the observed $r_0, r_1, n_0, n_1$, and where $F_{B(a_1+r_1,b_1+n_1-r_1)}$ is the cumulative distribution function of a beta distribution.
...
bap <- 1-integrate(function(x) (x)^(a_1+n_1*p_1-1)*(1-x)^(b_1+n_1-n_1*p_1-1)*pbeta(x-delta_star, a_0+n_0*p_0, b_0+n_0-n_0*p_0), lower = delta_star, upper = 1)[[1]]*(1/beta(a_1+n_1*p_1, b_1+n_1-n_1*p_1))
...
res_bap$bap <- round(res_bap$bap, 3)
