...
and thus
$$
P\left(\pi_1-\pi_0 \leq \delta^*\right)=\int_0^{1-\delta^*}\int_0^{\pi_0+\delta^*} f_2(\pi_1, \pi_0|r_0, r_1, n_0, n_1)d\pi_1d\pi_0=\int_0^{1-\delta^*}\frac{\pi_0^{a_0+r_0-1}(1-\pi_0)^{b_0+n_0-r_0-1}}{B(a_0+r_0, b_0+n_0-r_0)}\left(\int_0^{\pi_0+\delta^*}\frac{\pi_1^{a_1+r_1-1}(1-\pi_1)^{b_1+n_1-r_1-1}}{B(a_1+r_1, b_1+n_1-r_1)}d\pi_1\right)d\pi_0,
$$
is the Bayesian probability to reject. Grieve
...
inner_int <- Vectorize(function(y) {
    y^(a_0 + ceiling(n_0) * p_0 - 1) * (1 - y)^(b_0 +
...
bap <- integrate(inner_int, 0, 1 - delta_star)$value/(beta(a_1 +
    ...
    p_1) * beta(a_0 + ceiling(n_0) * p_0, b_0 + ceiling(n_0) -
    ceiling(n_0) * p_0))
...
# posterior_0 <- rbeta(10000, a_0 + n_0 * p_0, b_0 + n_0 -
#     n_0 * p_0)
# posterior_1 <- rbeta(10000, a_1 + n_1 * p_1, b_1 + n_1 -
#     n_1 * p_1)
# var_posterior_0 <- ((a_0 + n_0 * p_0) * (b_0 + n_0 -
#     n_0 * p_0))/((a_0 + b_0 + n_0)^2 * (a_0 + b_0 +
#     n_0 + 1))
...
#     n_1 * p_1))/((a_1 + b_1 + n_1)^2 * (a_1 + b_1 +
#     n_1 + 1))
# sum(posterior_1-posterior_0+qnorm(1-0.05)*sqrt(var_posterior_0+var_posterior_1)<=0.035)/10000

# s <- seq(max(ceiling(r_0+a_0-r_1-a_1),0), ceiling(r_0+a_0-1))

# sum(dhyper(s, m=ceiling(r_0+r_1+a_1+a_0-1), n=ceiling(n_1+n_0-r_1-r_0+b_1+b_0-1), k=ceiling(n_1+a_1+b_1-1)))
```
