...
\end{aligned}
$$ This is the **Bayesian probability to reject** (or Bayesian power) assuming one knows the true treatment effect $\delta$. Note that this is a conditional probability (given one knows $\delta$).
...
ggplot(data_power, aes(x = delta, y = y, colour = type)) + geom_line() +
    ...
    0.8, 1)) + ggtitle(str_glue("For treatment arm sample sizes: n0=",
    round(n_0, 0), ", n1=", round(n_1, 0))) + geom_vline(xintercept = 0,
    linetype = "dashed") + geom_vline(xintercept = 0.035, linetype = "dashed")
...
The Bayesian probability to reject for $\delta_A=0$ under an informative prior is 90%. This is higher than the frequentist probability to reject of 80%. In contrast, the probability to reject for $\delta_0=0.035$ under an informative prior is 11%. This is higher than the frequentist type-I error of 5%.
